...
test("vegetable production relay page", async ({ page }) => {
  ...
  const vegetableRelayHeader = page.getByRole("heading", {
    name: "野菜の産地リレー",
  });
...
test("employment page", async ({ page }) => {
  ...
  await page
    ...
    .click();
  const workersByGroupsHeader = page.getByRole("heading", {
    name: "年齢階級，教育，雇用形態別雇用者数(2002年～)",
  });
...
test("commodity prices page", async ({ page }) => {
  ...
  const commodityPricesHeader = page.getByRole("heading", {
    name: "小売物価統計調査（動向編）",
  });
